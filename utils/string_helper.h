...
inline string Left(const string& data, int len)
{
	if (len < 0)
	{
		len = 0;
...
inline string Right(const string& data, int len)
{
	if (len < 0)
	{
		len = 0;
	...
	return data.substr(data.size() - len, len);
}

inline string Left(const string& data, const char c)
{
	string::size_type pos = data.find(c);
	if (pos != string::npos)
	{
...
inline string Right(const string& data, const char c)
{
	string::size_type pos = data.rfind(c);
	if (pos != string::npos)
	{
